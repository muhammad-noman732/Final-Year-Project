        ...
        <div className="bg-[#1E293B]/60 backdrop-blur-2xl border border-white/5 shadow-2xl rounded-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-500">
          <div className="p-8 sm:p-10 space-y-8">
            <div className="space-y-1.5 text-center">
              <h2 className="text-2xl font-bold text-white tracking-tight leading-none">Update Password</h2>
              <p className="text-slate-400 text-sm font-medium leading-relaxed">Protect your account with a strong password</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder="••••••••"
                    className="h-12 rounded-xl bg-white/5 border-white/5 text-white focus:border-[#d4a843]/50 focus:ring-4 focus:ring-[#d4a843]/5 transition-all duration-200 px-4"
                ...
                {errors.currentPassword && <p className="text-[11px] font-bold text-red-400 ml-1">{errors.currentPassword.message}</p>}
              </div>
              ...
              <div className="space-y-2">
                ...
                {errors.newPassword && <p className="text-[11px] font-bold text-red-400 ml-1">{errors.newPassword.message}</p>}
                {newPassword.length > 0 && (
                  <div className="mt-3 p-3 rounded-xl bg-white/5 border border-white/5 space-y-2">
                    <div className="flex gap-1">
                      {[0,1,2,3].map(i => (
                        <div key={i} className={cn("h-1 flex-1 rounded-full transition-all duration-500", i < strengthScore ? (strengthColors as any)[strengthLevel] : "bg-white/10")} />
                      ))}
                    </div>
                    <p className="text-[9px] font-black uppercase tracking-widest text-slate-500">{(strengthLabels as any)[strengthLevel]} Password</p>
                  </div>
                )}
              </div>

              <div className="space-y-2">
                <Label className="text-[11px] font-black uppercase tracking-widest text-slate-500 ml-1">Confirm Password</Label>
                <div className="relative">
                  <Input
                ...
                {errors.confirmPassword && <p className="text-[11px] font-bold text-red-400 ml-1">{errors.confirmPassword.message}</p>}
                {passwordsMatch && <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mt-1 ml-1 animate-in fade-in">Match</p>}
              </div>

              <Button
                type="submit"
                disabled={isLoading}
                className="w-full h-12 bg-[#d4a843] hover:bg-[#e6b94d] text-[#0F172A] font-black text-sm tracking-wide transition-all duration-300 shadow-xl shadow-amber-950/20 rounded-xl relative overflow-hidden group"
              >
            ...
            <div className="pt-6 border-t border-white/5 text-center flex flex-col items-center gap-4">
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/5 border border-emerald-500/5">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-emerald-500/70">Secured</span>
              </div>
            </div>
          </div>
        </div>
