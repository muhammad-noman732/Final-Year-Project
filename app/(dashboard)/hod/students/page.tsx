...
import HodStudentsClient from "@/components/hod/HodStudentsClient"

async function StudentsLoader() {
  const { tenantId, userId } = await getTenantContext()
  const initialData = await hodService.getStudents(tenantId, userId, { page: 1, limit: 20 })
  return <HodStudentsClient initialData={initialData} />
}
  ...
  return (
    <div className="relative isolate min-h-[calc(100dvh-3.5rem)] p-5 lg:p-8" style={{ backgroundColor: "var(--hod-bg, #F4F6FA)" }}>
      ...
      <div className="mb-6 animate-pulse">
        <div className="h-7 w-48 bg-white/60 dark:bg-white/10 rounded-lg mb-2" />
        <div className="h-4 w-64 bg-white/40 dark:bg-white/5 rounded-md" />
      </div>
      ...
      <div className="relative overflow-hidden bg-white/40 dark:bg-white/5 rounded-xl shadow-sm border border-white/60 dark:border-white/10 animate-pulse">
        <div className="px-6 py-5 border-b border-white/60 dark:border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="h-5 w-40 bg-slate-100 dark:bg-slate-800 rounded-md" />
          </div>
          <div className="flex gap-3">
            <div className="h-9 w-48 bg-slate-100 dark:bg-slate-800 rounded-lg" />
        ...
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="px-6 py-5 border-b border-white/40 dark:border-white/5 flex items-center justify-between">
            ...
            <div className="h-4 w-12 bg-slate-50 dark:bg-slate-900 rounded" />
