...
const payMethods = [
    { method: "Visa (Stripe)", pct: 55, color: "#d4a843" },
    { method: "Mastercard (Stripe)", pct: 30, color: "#10b981" },
    { method: "Amex (Stripe)", pct: 10, color: "#0ea5e9" },
    { method: "Other Cards", pct: 5, color: "#f59e0b" },
];
        ...
        <div className="space-y-5 pb-8">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground/60">· Updated {lastUpdated}s ago</span>
                </div>
                <div className="flex gap-2">
                    {["PDF", "CSV", "Excel"].map((f) => (
                        <Button key={f} size="sm" variant="outline" className="h-7 text-[11px] px-2.5 border-white/[0.07] text-slate-500 hover:text-gold-400 hover:border-gold-500/20">
                            {f}
                        </Button>
                    ))}
                </div>
            ...
            <div className="flex flex-wrap items-center gap-2 px-4 py-3 rounded-xl border border-white/[0.05] bg-[#0a0e1a]">
                ...
                <button className="ml-auto text-xs text-muted-foreground/60 hover:text-gold-400 transition-colors">Reset</button>
            </div>
                ...
                {deptCards.map((d) => (
                    <div key={d.name} className="rounded-2xl border border-white/[0.05] bg-gradient-to-br from-[#0d1321] to-[#080c14] p-5 hover:border-gold-500/12 transition-all duration-300 hover:-translate-y-0.5 group">
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-sm font-bold text-foreground tracking-tight">{d.name}</h3>
                            <div className="flex items-center gap-2">
                                <span className="text-[11px] px-2 py-0.5 rounded-full border border-gold-500/20 text-gold-400 bg-gold-500/5 uppercase tracking-wider font-bold">{d.short}</span>
                                <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${d.rate >= 95 ? "text-emerald-400 bg-emerald-500/8 border border-emerald-500/15" : d.rate >= 90 ? "text-amber-400 bg-amber-500/8 border border-amber-500/15" : "text-rose-400 bg-rose-500/8 border border-rose-500/15"}`}>{d.rate}%</span>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-4 text-center">
                            {[
                                ...
                                <div key={item.label} className="rounded-lg bg-white/[0.02] border border-white/[0.04] py-2 px-1">
                                    ...
                                    <p className="text-[11px] text-muted-foreground/60 uppercase tracking-wider mt-0.5">{item.label}</p>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="space-y-2 mb-3">
                            ...
                            {d.defaulters > 0 && (
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[11px] text-rose-400">{d.defaulters} defaulters</span>
                                </div>
                            )}
                        </div>
            ...
            <div className="grid lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-1 rounded-2xl border border-white/[0.05] bg-[#0a0e1a] p-5">
                    <div className="flex items-center justify-between mb-4">
                        <h4 className="text-xs font-semibold text-foreground tracking-tight">30-Day Trend</h4>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3 text-center">
                        {[
                            ...
                            <div key={s.label} className="rounded-lg bg-white/[0.02] py-1.5 px-1">
                                ...
                                <p className="text-[11px] text-muted-foreground/60 uppercase tracking-wider">{s.label}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                {/* Bar Chart */}
                <div className="lg:col-span-1 rounded-2xl border border-white/[0.05] bg-[#0a0e1a] p-5">
                    <h4 className="text-xs font-semibold text-foreground tracking-tight mb-4">Today vs Yesterday</h4>
                    <ResponsiveContainer width="100%" height={200}>
                ...
                <div className="lg:col-span-1 rounded-2xl border border-white/[0.05] bg-[#0a0e1a] p-5">
                    <h4 className="text-xs font-semibold text-foreground tracking-tight mb-4">Collection Rate by Dept</h4>
            ...
            <div className="grid lg:grid-cols-3 gap-4">
                ...
                <div className="rounded-2xl border border-white/[0.05] bg-[#0a0e1a] p-5">
                    <h4 className="text-xs font-semibold text-foreground tracking-tight mb-2">Payment Methods</h4>
                ...
                {/* Stacked Bar */}
                <div className="rounded-2xl border border-white/[0.05] bg-[#0a0e1a] p-5">
                    <h4 className="text-xs font-semibold text-foreground tracking-tight mb-4">Collection by Semester</h4>
                ...
                <div className="rounded-2xl border border-white/[0.05] bg-[#0a0e1a] overflow-hidden">
                    <div className="px-4 py-3.5 border-b border-white/[0.04] flex items-center justify-between">
                        ...
                        <span className="text-[11px] text-muted-foreground/60">every 3s</span>
                    </div>
                            ...
                            {liveEvents.map((e) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[11px] text-slate-500">{e.dept} · {e.sem} Sem</p>
                                    </div>
                                    <div className="text-right flex-shrink-0">
                                        <p className="text-xs font-semibold text-gold-400">{formatFullCurrency(e.amount)}</p>
                                        <p className="text-[11px] text-muted-foreground/60">{e.time}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
