...
import HodDefaultersTable from "@/components/hod/HodDefaultersTable"
import type { HodDashboardClientProps } from "@/types/client/ui/hod.ui.types"

export default function HodDefaultersClient({ initialData }: HodDashboardClientProps) {
  const { defaulters, isLoading } = useHodDashboard(initialData, { fetchDashboard: true, fetchStudents: false })
    ...
    <div className="relative isolate min-h-[calc(100dvh-3.5rem)] p-5 lg:p-8 transition-colors duration-300" style={{ backgroundColor: "var(--hod-bg, #F4F6FA)" }}>
      ...
      <div className="mb-6 text-left">
        ...
        <p className="text-sm text-[#64748B] dark:text-slate-400 mt-1.5 leading-none">
          Track and manage department fee defaulters
        </p>
      </div>

      <Skeleton name="hod-defaulters-table" loading={isLoading && defaulters.length === 0}>
        <HodDefaultersTable defaulters={defaulters} />
      </Skeleton>
    </div>
  )
}
